...
with st.container():
    ...
    st.text("O projeto de captura de trepidação em ambiente doméstico consiste no desenvolvimento de  um dispositivo capaz \nde realizar a captura de vibração em um ambiente doméstico, registrando os possíveis momentos de pico e como influencia na vida de moradores. Tem por objetivo: O registro de dados de vibração em \nambiente doméstico e possíveis momentos de pico, como por exemplo, em uma moradia que esteja próximo a terreno com obras ou próximo a avenidas ou região com movimento de tráfego intenso.")
